...
所要時間の目安: 字幕取得が動画数×1〜2秒、タグ付けがGemini無料枠レート制御のため動画数×4.5秒(全件で数時間。`--skip-tags` で省略可、後から `python ingestion/tag_videos.py` で追加可能)、埋め込みはCPUで数十分。すべて**再実行可能(取得・付与済みはスキップ)**なので、中断しても再開できます。
